...
# reweight tri2 
bash steps/decode-ccnc-arg.sh --nj $decode_nj --skip_first_pass true --filter_likes false --use_landmark true --run_reweight true --REWEIGHT_FACTOR $reweight_fact --drop_mod 0 --fill_mod 0 --perc 0 --repeat_stop 0 --ampl_fact 0 exp/tri2/graph/ data/test exp/tri2/decode_test >> $log_running
...
bash steps/decode-ccnc-arg.sh --nj $decode_nj --skip_first_pass true --filter_likes true --DECIMATION_RATIO $decimate_ratio --use_landmark true --run_reweight false --REWEIGHT_FACTOR 0 --drop_mod 4 --fill_mod 5 --perc 0 --repeat_stop 0 --ampl_fact 1.75 exp/tri2/graph/ data/test exp/tri2/decode_test >> $log_running

echo 'dropping-frames by "${decimate_ratio}", avoiding landmarks:' >> $log_result
grep 'Mean*' ./exp/tri2/decode_test/score_*/ctm_39phn.filt.sys >> $log_result

# randomly drop some frames
bash steps/decode-ccnc-arg.sh --nj $decode_nj --skip_first_pass true --filter_likes true --DECIMATION_RATIO 0 --use_landmark true --run_reweight false --REWEIGHT_FACTOR 0 --drop_mod 2 --fill_mod 2 --perc $drop_perc --repeat_stop 0 --ampl_fact 0 exp/tri2/graph/ data/test exp/tri2/decode_test >> $log_running
...
steps/decode-ccnc-nnet-arg.sh --cmd $decode_cmd --nj $decode_nj --filter_likes true --DECIMATION_RATIO 0 \
  ...
  exp/tri4_nnet/decode_test | tee $log_running
