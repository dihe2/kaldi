...
#FILT_LL=$BINDIR/decimate-likelihoods-phoneMark2.pl
...
#	likelihoods="ark:nnet-am-compute --apply-log=$APPLY_LOG $model '$feats' ark,t:- | perl $FILT_LL --decimation-ratio $DECIMATION_RATIO |"
	#likelihoods="ark:nnet-am-compute --apply-log=$APPLY_LOG $model '$feats' ark,t:- | perl $FILT_LL --upsample-ratio $DECIMATION_RATIO |"
	#likelihoods="ark:nnet-am-compute --apply-log=$APPLY_LOG $model '$feats' ark,t:- | perl $FILT_LL --decimation-ratio $DECIMATION_RATIO --avoid-boundary $BOUNDARY_DISTANCE --save-compression-ratio $dir/compr_ratio.JOB.txt $dir/phone_info.JOB.txt |"
	#likelihoods="ark:nnet-am-compute --apply-log=$APPLY_LOG $model '$feats' ark,t:- | perl $FILT_LL --reweight-factor $REWEIGHT_FACTOR --save-reweight-ratio $dir/reweight_marks_ratio.JOB.txt $dir/phone_mark_info.JOB.txt |"
  if $use_landmark; then
	if $run_reweight; then
		likelihoods="ark:nnet-am-compute --apply-log=$APPLY_LOG $model '$feats' ark,t:- | perl $FILT_LL --reweight-factor $REWEIGHT_FACTOR --save-reweight-ratio $dir/reweight_marks_ratio.JOB.txt $dir/phone_mark_info.JOB.txt |"
	else
		echo "DEC_LANDMARK"
		likelihoods="ark:nnet-am-compute --apply-log=$APPLY_LOG $model '$feats' ark,t:- | perl $FILT_LL --decimation-ratio $DECIMATION_RATIO --save-compression-ratio $dir/compr_mark_ratio.JOB.txt --drop-mod $drop_mod --fill-mod $fill_mod --perc $perc --repeat-stop $repeat_stop --ampl-fact $ampl_fact $dir/phone_mark_info.JOB.txt |"
	fi
  else
	likelihoods="ark:nnet-am-compute --apply-log=$APPLY_LOG $model '$feats' ark,t:- | perl $FILT_LL --decimation-ratio $DECIMATION_RATIO --drop-mod $drop_mod |"
  fi
	#likelihoods="ark:nnet-am-compute --apply-log=$APPLY_LOG $model '$feats' ark,t:- |"

	$cmd $parallel_opts JOB=1:$nj $dir/log/decode.2.JOB.log \
	  latgen-faster-mapped --minimize=$minimize --acoustic-scale=$acwt \
	        --allow-partial=true --min-active=$min_active \
